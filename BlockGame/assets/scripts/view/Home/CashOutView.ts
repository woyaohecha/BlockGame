...
export class CashOutView extends Component {
    ...
    onBtnCashOut() {
        ...
        if (this._selectItem.getComponent(CashItem).restCount == 0) {
            TipsManager.getInstance().showTips("剩余提现次数不足");
            return;
        }
        if (this._hasPassword) {
            console.log("有密码", this._selectItem.getComponent(CashItem).cashId);
            UserData.getInstance().selectCashId = this._selectItem.getComponent(CashItem).cashId;
