...
@ccclass('BlockLogic')
export class BlockLogic extends Component {
     ...
     */
    checkGridPanel() {
        let gridPanelLogic = find("Canvas/Play/GridPanel").getComponent(GridPanelLogic);
     ...
     */
    checkNextEmpty() {
        let nextPanelLogic = find("Canvas/Play/NextPanel").getComponent(NextPanelLogic);
    ...
    place() {
        ...
        if (result) {
            for (let child of result.canPutGrid) {
                child.getComponent(GridLogic).setImg(result.spriteFrame)
            }
            BlockPool.getInstance().removeBlock(this.node);
            let gridPanelLogic = find("Canvas/Play/GridPanel").getComponent(GridPanelLogic);
    ...
    setGray() {
        this.canDrag = false;
        BlockPool.getInstance().getImg(7, (sp: SpriteFrame) => {
            for (let child of this.node.children) {
